...
RiPPsM <-subset(BGCStat,select=RiPPs) 
NRPSM <-subset(BGCStat,select=NRPSc) 
PKSM <-subset(BGCStat,select=PKS) 
TerpeneM <-subset(BGCStat,select=c(terpene)) 
OthersM <-subset(BGCStat,select=Others) 

head(NRPSM)

p0 <- ggtree(tree,size = 0.1 ,layout="circular")   + 
  geom_hilight(data=bgPhylum, mapping=aes(node=node,fill = phylum),extendto=1.7, alpha=0.2, color=NA,size=0.05,show.legend=FALSE)+
...
p2 <- gheatmap(p1, phylumInfo,offset=-0.05, width=0.05,colnames_angle=45, colnames_offset_y = 0 ,font.size = 1.3,color =NA,hjust=1)+
...
p5 <- gheatmap(p3, RiPPsM,offset=0.01, width=0.02*9,colnames_angle=45, colnames_offset_y = 0 ,font.size = 1.3,color =NA,hjust=1)+
  scale_color_manual(values=c("#00441b","#238b45","#41ab5d","#74c476","#ffffff"	),limits = c(">2","1-2","0.5-1","0-0.5","0"),name="RiPPs",guide=guide_legend(legend.key.size = unit(0.2, 'cm'),order=2) ) +
  scale_fill_manual(values=c("#00441b","#238b45","#41ab5d","#74c476","#ffffff"	),limits = c(">2","1-2","0.5-1","0-0.5","0"),name="RiPPs",guide=guide_legend(legend.key.size = unit(0.2, 'cm'),order=2) ) 


#NRPS #543005 #d95f03	#fc8d62	#fdcdac
p6  <- p5 + new_scale_fill() + new_scale_color()
p7 <- gheatmap(p6, NRPSM,offset=0.20, width=0.02*2,colnames_angle=45, colnames_offset_y = 0 ,font.size = 1.3,color =NA,hjust=1)+
  scale_color_manual(values=c("#7f3b08","#b35806","#e08214","#fdb863","#ffffff"	),limits = c(">2","1-2","0.5-1","0-0.5","0"),name="NRPS",guide=guide_legend(legend.key.size = unit(0.2, 'cm'),order=3) ) +
  scale_fill_manual(values=c("#7f3b08","#b35806","#e08214","#fdb863","#ffffff"	),limits = c(">2","1-2","0.5-1","0-0.5","0"),name="NRPS",guide=guide_legend(legend.key.size = unit(0.2, 'cm'),order=3) ) 



#PKS #3f007d #7570b4	#8da0cb	#cbd5e8
p8  <- p7 + new_scale_fill() + new_scale_color()
p9 <- gheatmap(p8, PKSM,offset=0.245, width=0.02*4,colnames_angle=45, colnames_offset_y = 0 ,font.size = 1.3,color =NA,hjust=1)+
  scale_color_manual(values=c("#2d004b","#542788","#8073ac","#b2abd2","#ffffff"	),limits = c(">2","1-2","0.5-1","0-0.5","0"),name="PKS",guide=guide_legend(legend.key.size = unit(0.2, 'cm'),order=4) ) +
  scale_fill_manual(values=c("#2d004b","#542788","#8073ac","#b2abd2","#ffffff"	),limits = c(">2","1-2","0.5-1","0-0.5","0"),name="PKS",guide=guide_legend(legend.key.size = unit(0.2, 'cm'),order=4) ) 


#Terpene #8e0152 #e7299a	#e78ac3	#f4cae4
p10  <- p9 + new_scale_fill() + new_scale_color()
p11 <- gheatmap(p10, TerpeneM,offset=0.34, width=0.02,colnames_angle=45, colnames_offset_y = 0 ,font.size = 1.3,color =NA,hjust=1)+
  scale_color_manual(values=c("#8e0152","#c51b7d","#de77ae","#f1b6da","#ffffff"	),limits = c(">2","1-2","0.5-1","0-0.5","0") ,name="Terpene",guide=guide_legend(legend.key.size = unit(0.2, 'cm'),order=5) ) +
  scale_fill_manual(values=c("#8e0152","#c51b7d","#de77ae","#f1b6da","#ffffff"	),limits = c(">2","1-2","0.5-1","0-0.5","0"),name="Terpene",guide=guide_legend(legend.key.size = unit(0.2, 'cm'),order=5)  ) 


#Others #000000 #252525	#636363	#cccccc
p12  <- p11 + new_scale_fill() + new_scale_color()
p13 <- gheatmap(p12, OthersM,offset=0.37, width=0.02*16,colnames_angle=45, colnames_offset_y = 0 ,font.size = 1.3,color =NA,hjust=1)+
  scale_color_manual(values=c("#1a1a1a","#4d4d4d","#878787","#bababa","#ffffff"	),limits = c(">2","1-2","0.5-1","0-0.5","0") ,name="Others",guide=guide_legend(legend.key.size = unit(0.2, 'cm'),order=6) ) +
  scale_fill_manual(values=c("#1a1a1a","#4d4d4d","#878787","#bababa","#ffffff"	),limits = c(">2","1-2","0.5-1","0-0.5","0") ,name="Others",guide=guide_legend(legend.key.size = unit(0.2, 'cm'),order=6) ) 
...
pdf("tree.BGC.pdf",width=12,height=10,onefile = FALSE)
p15
dev.off()


png("tree.BGC.png",width=12,height=10,units='in',res=300)
p15
dev.off()